...
export const getConversations = (
  ...
  const user1Conversations = conversations.filter(
    (conversation: any) => conversation.id === user1Id
  );
  const user2Conversations = conversations.filter(
    (conversation: any) => conversation.id === user2Id
  );

  const user1Messages =
    user1Conversations.length > 0 ? user1Conversations[0].messages : [];
  const user2Messages =
    user2Conversations.length > 0 ? user2Conversations[0].messages : [];

  const allMessages = [...user1Messages, ...user2Messages].sort(
    (a: any, b: any) => (a.datetime < b.datetime ? -1 : 1)
  );
...
export const counterOfferMessage = (messageObj: any) => {
  ...
  if (messageObj.isWithCounterOffer) {
    ...
    messageObj = {
      ...messageObj,
      message: `Counter Offer: ${text[text.length - 1]}$`,
    };
  }
