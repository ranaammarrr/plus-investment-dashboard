...
export const getAllCategories = createAsyncThunk<Categories[], void,{ rejectValue: ApiError }>(
    'timeline/all',
...
export const createCategories = createAsyncThunk<Categories, Body ,{ rejectValue: ApiError }>(
    'category/create',
    async ({label, icon}, { rejectWithValue })  => { 

        const body = {label: label, icon: icon}
        try {
        const response = await fetch(`${BASE_URL}${ENDPOINTS.POST_ALL_CATEGORIES}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization' : `Bearer ${token}`
...
//   Delete Category..... 
  ...
  export const deleteCategory = createAsyncThunk<void, string, { rejectValue: ApiError }>(
    'properties/delete',
    async (category_Id, { rejectWithValue }) => {
      try {
        const response = await fetch(`${BASE_URL}${ENDPOINTS.DELETE_CATEGORY}/${category_Id}`, {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
            'Authorization' : `Bearer ${token}`
