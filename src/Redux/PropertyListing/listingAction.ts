  ...
  try {
    ...
    if (!apiResponse.success) {
      ...
      return rejectWithValue({
        message: `Cannot delete property with ID ${property_Id}`,
      });
    }
...
export const getPropertyDetail = createAsyncThunk<
  ...
  string,
  { rejectValue: ApiError }
>("properties/getPropertyDetail", async (property_Id, { rejectWithValue }) => {
  try {
    const response = await fetch(
      `${BASE_URL}${ENDPOINTS.GET_PROPERTY_DETAIL}/${property_Id}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
