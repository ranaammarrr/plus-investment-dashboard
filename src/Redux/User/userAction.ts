...
>("user/delete", async (userId, { rejectWithValue }) => {
  try {
    const response = await fetch(
      `${BASE_URL}${ENDPOINTS.DELETE_USER}${userId}`,
      {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
...
export const isVerifiedUser = createAsyncThunk<
  void,
  { user_id: string; isVerified: string },
  { rejectValue: ApiError }
>("properties/approved", async ({ user_id }, { rejectWithValue }) => {
  try {
    const response = await fetch(
      `${BASE_URL}${ENDPOINTS.IS_VERIFIED_USER}/${user_id}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
